...
STOP_COLOR = 'red2'
TARGET_AREA =100
TARGET_BLINK_COLOR = 'yellow2'
TARGET_BLINK_COLOR_2 = 'black'
...
class App:
	...
	def start_begin_now(self, frame, f_direction, check_position):
		...
		# exit for-loop if flag set
			...
			#Place frame when changing direction
				...
				frame.place(x = zo.win_x, y = zo.win_y)
				if check_position():
					zo.a[UP].config(bg = 'maroon1')
				else:
					zo.a[UP].config(bg = BLINK_COLOR)
			...
			time.sleep(0.3)
			zo.a[UP].config(bg = lbbg)
			zo.a[DOWN].config(bg = lbbg)
			#time.sleep(0.15)
	...
	def check_if_stopped_at_target(self, check_position):
		...
		while zo.stopped_at_target:
			if check_position():
				for n in ( LEFT, RIGHT): #UP, DOWN,
					zo.a[n].config(bg = TARGET_BLINK_COLOR)
				if not zo.stopped_at_target : break
				time.sleep(0.5)
				for n in ( LEFT, RIGHT): #UP, DOWN,
					zo.a[n].config(bg = TARGET_BLINK_COLOR_2)
				if not zo.stopped_at_target : break
				time.sleep(0.5)
	...
	def reset_now(self):
		...
		zo.exit_thread = True
		zo.stopped_at_target = False
		time.sleep(0.3) # Time for the task to end
		self.reset_frame()
		self.do_buttons()
